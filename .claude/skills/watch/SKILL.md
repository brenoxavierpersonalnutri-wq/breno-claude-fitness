...
name: watch
version: "0.2.0"
description: "ASSISTE um vídeo de verdade (URL ou arquivo local) — baixa com yt-dlp, extrai frames com ffmpeg e puxa a transcrição com timestamp (legendas nativas ou Whisper), e responde vendo o que está NA TELA, não só o que foi dito. Use quando o pedido envolver: assistir vídeo, ver vídeo, analisar vídeo, o que aparece no vídeo, o que ele mostra na tela, engenharia reversa de Reels/VSL/anúncio, analisar criativo de concorrente, quebrar a estrutura de um vídeo, ver o gancho dos primeiros segundos, analisar slide/print/demo dentro do vídeo, diagnosticar bug em gravação de tela, resumir vídeo longo, o que acontece em tal minuto. Também roda em .mp4/.mov/.mkv/.webm local. FRONTEIRA no Squad Turbo: se o pedido é SÓ o texto falado (transcrever aula, pegar a transcrição, minerar fala do expert), prefira transcrever-youtube-turbo; se é BUSCAR vídeos, canais ou playlists, prefira youtube-full. Esta skill é a única que enxerga a IMAGEM — use quando o visual importa (slides, demonstração, edição, corte, expressão, o que está escrito na tela). Custo alto de tokens: cada frame é uma imagem — em vídeo longo, focar um trecho com --start/--end. SEM CHAVE DE WHISPER: quando o vídeo não tem legenda, use o fallback LOCAL (whisper-local/ dentro desta skill · faster-whisper na CPU · o áudio não sai da máquina) — ver seção 'Fallback local' no corpo."
...
## Fallback local — transcrição SEM chave de API (padrão neste ambiente)

Neste ambiente NÃO há chave de Whisper configurada, de propósito: a transcrição
sem legenda é feita LOCALMENTE com faster-whisper (mesmo motor da skill
cortes-tiktok-turbo · int8 · CPU · o áudio nunca sai da máquina).

Quando o relatório do watch.py vier com "No transcript available" (vídeo sem
legenda, ou arquivo local):

1. Rode o watch.py normalmente com `--no-whisper` (gera os frames e baixa o
   vídeo pro work dir).
2. Transcreva o próprio vídeo baixado (aceita mp4/mov/webm direto):

```bash
~/.claude/skills/watch/whisper-local/venv/bin/python \
  ~/.claude/skills/watch/whisper-local/transcrever-local.py \
  "<work-dir>/video.mp4" --model medium
```

3. A saída são linhas `[MM:SS] texto` — o mesmo formato do relatório. Junte com
   os frames e responda como sempre.

Notas:
- **venv não existe?** (instalação nova via zip do squad — o zip não carrega os
  218 MB): rode uma vez `bash ~/.claude/skills/watch/whisper-local/instalar.sh`
  e siga normalmente.
- `--start MM:SS --end MM:SS` transcreve só o trecho (casa com o modo focado).
- `--model tiny` para triagem rápida de vídeo longo; `medium` (padrão) para o
  resultado final. Modelos já baixados em `~/.cache/huggingface`.
- ~1-2 min de processamento por 10 min de áudio no Apple Silicon. Pra vídeo
  longo, avise o usuário que a transcrição local está rodando.
- Se um dia houver chave no `~/.config/watch/.env`, o fluxo da API volta a
  valer e este fallback fica como opção de privacidade.
